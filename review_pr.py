...
def build_prompt(
    ...
    parts.append(
        ...
        f"{diff_prompt}\n\n"
        f"Apply the review skill in your instructions. For every finding whose "
        f"complete fix fits at the anchored line(s), include the `suggestion` "
        f"field with the exact replacement code so it becomes a one-click "
        f"suggested change; an insertion counts too (replace the anchored line "
        f"with the added code plus the original line). Return ONLY the JSON "
